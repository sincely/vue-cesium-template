...
export default class entityFactory {
  constructor(obj) {
    ...
    this.options = null
    switch (obj.type) {
      case 'triangleMeasure':
        this.createTriangleMeasure(obj.data)
        break
      case 'createLine':
        this.createLine(obj.data)
        break
      case 'dynamicCylinder':
        this.dynamicCylinder(obj.data)
        break
      case 'createPolygon':
        this.createPolygon(obj.data)
        break
      case 'createScan':
        this.createScan(obj.data)
        break
    }
    return this.options
  }
  ...
  createTriangleMeasure(poly) {
    ...
    var _text = function () {
      if (typeof _self.label.fn == 'function') {
        //fn 转换函数 scaler 换算  unit 单位
  ...
  createScan(opt) {
    let _self = this,
      viewer = opt.v,
      length = opt.positions.alt
    _self.entity = opt.entity
    _self.modelMatrix = opt.modelMatrix
    _self.positions = opt.positions
    _self.Primitives = new primitives(viewer)
    // 4 创建雷达放射波
    ...
    let _update = function () {
      let positions = _self.entity.position.getValue(viewer.clock.currentTime)

      let modelMatrix = _self.Primitives.countModelMatrix(positions)

      _self.modelMatrix = modelMatrix
