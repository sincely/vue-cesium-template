...
export default class Scene {
  ...
  grid(opt) {
    ...
    if (opt.cmd == _self.STATUS.CREATE) {
      ...
      _self.layer = _self.imageryLayers.addImageryProvider(mapGrid) //添加注记图层
      //_self.imageryLayers.grid.data = layer;
      _self.imageryLayers.raiseToTop(_self.layer) //将注记图层置顶
    }
    if (opt.cmd == _self.STATUS.CLOSE) {
      _self.imageryLayers.remove(_self.layer)
    }
    //return _self.layer;
  }
  /**
  ...
  mgrsGrid(opt) {
    ...
    if (opt.cmd == _self.STATUS.CREATE) {
      ...
      let pn = Cesium.KmlDataSource.load(
        'data/EarthPointGrid_093357.kml',
        options
      )
